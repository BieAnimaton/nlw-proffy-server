...
class ClassesController {
    async index(request, response) {
        ...
        if (!filters.week_day || !filters.subject || !filters.time) {
            return response.status(400).json({
                error: 'Missing filtes to search classes'
            });
        }
    ...
    async create(request, response) {
        const { 
        /* apenas name, pois as variáveis possuem
        nomes iguais, caso contrário, name: nome*/
        name, avatar, whatsapp, bio, subject, cost, schedule } = request.body;
